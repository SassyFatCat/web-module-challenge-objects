...
const salad = createMenuItem("Garden Salad", 4, "Lunch");
const omelette = createMenuItem("Cheese Omelette", 5, "Breakfast");
...
burger.discount = (whoIsIt) => whoIsIt == "teacher" | whoIsIt == "student" ? burger.price * .75
                                                                            : burger.price * .9;
...
function getReviewByRating(arr, rating) {
  let rangedReviews = [];
  arr.forEach(element => element.rating >= rating ? rangedReviews.push(element) : null)
...
*/
  ...
  let rangedReviews = [];  
  arr.forEach(element => element["feedback"].split(' ').length > 15 ? rangedReviews.push(element) : null)
  return rangedReviews;
  }
